...
const MediaPreview = ({ media, isWinner, isSelected, onClick, percentage, option, totalVotes, userVote, fullScreen = false }) => {
  ...
  console.log(`MediaPreview Debug - Option ${option?.id}: isMobile=${isMobile}, userVote=${userVote}, shouldShowBars=${shouldShowBars}, displayPercentage=${displayPercentage}`);
  ...
  if (!media) {
    return (
      ...
      >
        ...
        {/* Percentage Display - Only show when user has voted on mobile */}
        {shouldShowBars && (
          <motion.div 
  ...
  if (media.type === 'video') {
    return (
      ...
      >
        ...
        {/* Percentage Display for videos - Only show when user has voted on mobile */}
        {shouldShowBars && (
          <motion.div 
            className="absolute top-2 right-2 bg-black/70 text-white px-2 py-1 rounded-full text-xs font-bold z-10"
            initial={{ opacity: 0, scale: 0.8 }}
                ...
                <button
                  className={cn(
                    ...
                    authorHasStories && authorStoriesData?.has_unviewed
                      ? "w-10 h-10 rounded-full overflow-hidden p-[2px] bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600"
                      : authorHasStories && !authorStoriesData?.has_unviewed
                      ? "w-10 h-10 rounded-full overflow-hidden p-[2px] bg-gray-400"
                      : ""
                  )}
