...
const LogoWithQuickActions = ({ size = 32, className = "" }) => {
  ...
  return (
    <>
      <div
        {...longPressProps}
        className={`${className} relative flex items-center justify-center cursor-pointer select-none transition-all duration-300 rounded-full ${
          isPressed 
            ? 'scale-110 shadow-2xl bg-black' 
            : 'hover:scale-105 hover:shadow-lg bg-black'
        }`}
        onMouseDown={(e) => {
      ...
      >
        ...
        {isPressed && (
          <>
            <div className="absolute inset-0 rounded-full bg-black/40 animate-ping" style={{zIndex: -1}} />
            <div className="absolute inset-0 rounded-full bg-black/30 animate-ping" style={{zIndex: -2, animationDelay: '150ms'}} />
            <div className="absolute inset-0 rounded-full bg-black/20 animate-ping" style={{zIndex: -3, animationDelay: '300ms'}} />
          </>
        )}
